      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
        <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 min-w-0">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-slate-900 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0">
              <RiBook2Fill className="text-white text-lg sm:text-xl" />
            </div>

            <div className="min-w-0 flex-1">
              <p className="font-semibold text-slate-900 text-sm sm:text-base leading-tight mb-1 sm:mb-0 line-clamp-2 sm:line-clamp-1">
                {course.courseName}{' '}
                <span className="text-[11px] sm:text-xs text-slate-500 font-normal inline-block mt-0.5 sm:mt-0">({course.courseCode})</span>
              </p>

              <div className="mt-1 sm:mt-1.5 flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs text-slate-600">
                {hasActiveSession && activeSession ? (
                  <>
                    <span className="inline-flex items-center gap-1 bg-slate-50 px-1.5 py-0.5 rounded">
                      <FiClock className="text-slate-500 w-3 h-3" />
                      {timeText}
                    </span>
          ...
          <div className="shrink-0 w-full sm:w-auto flex justify-end mt-1 sm:mt-0">
            {hasActiveOutpass ? null : canShowMarkButton ? (
              <button
                onClick={() => setShowScanner(true)}
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 sm:py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition-colors"
              >
                <IoQrCodeOutline className="text-lg" />
                Mark attendance
              </button>
            ) : readOnly ? (
              <div className="w-full sm:w-auto text-center px-4 py-2 sm:py-2.5 rounded-xl bg-slate-100 text-slate-600 text-xs sm:text-sm font-semibold">
                View only
              </div>
            ) : (
              <div className="w-full sm:w-auto text-center px-4 py-2 sm:py-2.5 rounded-xl bg-slate-50 text-slate-500 text-xs sm:text-sm font-medium border border-slate-200">
